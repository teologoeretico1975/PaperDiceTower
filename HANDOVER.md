...
- **Se un giorno si vuole una versione colorata**, la scelta e' *dove* si dipinge, e non e' libera. Sul cartamodello 2D serve prima **un layout a orientamento uniforme**: oggi la verticale della torre punta a 0 gradi sul corpo principale e a 90 sul pezzo grande del fusto, quindi qualsiasi cosa abbia un "sotto" (colature, muschio, ombreggiatura) uscirebbe ruotata fra un pezzo e l'altro. Non e' un limite della geometria ma dell'impaginazione, e si paga in pagine. In 3D il problema non esiste, ma servono UV e materiali riattivati. Analisi completa in `memory/reference_prodotto_simile.md`.
