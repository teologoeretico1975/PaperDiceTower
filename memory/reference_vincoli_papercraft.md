...
**Rigidezza: corrugare invece di irrobustire la carta.** La rigidezza a flessione cresce col cubo dell'altezza della sezione, e in un foglio piatto quell'altezza e' lo spessore della carta. Un pezzo incollato su un solo lato e proteso nel vuoto flette e finisce per piegarsi sulla linea di colla, per quanto si aumenti la grammatura. Corrugando la striscia (pieghe a fisarmonica) l'altezza utile passa da 0,25 a ~5 mm e il problema scompare senza cambiare materiale. Vincolo: **le pieghe devono correre lungo la luce**; trasversali darebbero un soffietto, cioe' una molla, piu' cedevole di un foglio piatto. In piu' un pezzo che va da parete a parete e' appoggiato a entrambi gli estremi invece che a sbalzo.

Corollario: una superficie non sviluppabile (un elicoide, per esempio una scala a chiocciola) non e' un'opzione, perche' non si appiattisce su un foglio senza stirare la carta e andrebbe approssimata con molte faccette.

**Criteri opposti da misurare, non da stimare.** Per un ostacolo interno che deve intercettare un oggetto in caduta senza bloccarlo servono due verifiche contrarie: l'insieme degli ostacoli non deve lasciare un canale libero piu' largo dell'oggetto **piu' piccolo**, e ogni ostacolo da solo deve lasciare un varco piu' largo dell'oggetto **piu' grande**. Ottimizzare solo il primo produce un imbuto che si intasa. Su questo il ragionamento a mano mi ha ingannato due volte (una stima di copertura del 62% era in realta' 49%, e una configurazione a tre ostacoli non migliorava nulla): va misurato con un ray-cast su una griglia della sezione.
